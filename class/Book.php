...
class Book
{
    ...
    private static $rootSQLPage = "select b.*,
            c.name as category_name,
            a.name as author_name,
            p.name as publisher_name,
            s.name as series_name
    ...
    {
        $sql = sprintf(self::$rootSQLPage, "(select *  from book where book.discount is not null)");
    ...
    {
        $sql = sprintf(self::$rootSQLPage, "(select *  from book where book.category_id = :category_id)");
     ...
     static public function getAdminPage($conn, $limit, $offset)
    {
        $sql = sprintf(self::$rootSQLPage, "(select *  from book)");
    ...
    public function updateBook($conn)
    {
        if (self::validateBook($conn)) {
            ...
            $this->fillBookStatement($statement);
    ...
    private function fillBookStatement($statement)
    {
    ...
    public static function getTotalOfBooks($conn)
    {
